...
def connect_to_port():
    # try to find the correct serial port starting with the name 'USB VCOM For Protocol'
    ...
    while not com_port_found:
        ...
        for port in ports:
            if port.description.find('USB VCOM For Protocol') != -1:
                ...
                found_serial_port = serial.Serial(port=port_name)
                # set color to green
                print('\u001b[32;1m')
...
def read_controller(serial_port):
    ...
    thread = threading.Thread(target=threaded_function, args=(), daemon=True)
    thread.start()
...
while True:
    port_connection = connect_to_port()
    read_controller(port_connection)
    time.sleep(1)
